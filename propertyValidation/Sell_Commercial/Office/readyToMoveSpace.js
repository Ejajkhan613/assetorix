...
// Function
function readyToMoveSpace(data) {
    ...
    let officeSetup = component.officeSetup(data.officeSetup);
    if (officeSetup.msg == "SUCCESS") {
        obj.officeSetup = officeSetup.data;
    } else {
        return officeSetup;
    }
    ...
    // Conference Room
    let conferenceRoom = component.conferenceRoom(data.conferenceRoom);
    if (conferenceRoom.msg == "SUCCESS") {
        obj.conferenceRoom = conferenceRoom.data;
    } else {
    ...
    // Reception Area
    let receptionArea = component.receptionArea(data.receptionArea);
    if (receptionArea.msg == "SUCCESS") {
        obj.receptionArea = receptionArea.data;
    } else {
    ...
    let facilityAvailableMore = component.facilityAvailableMore(data.facilityAvailable);
    if (facilityAvailableMore.msg == "SUCCESS") {
        obj.washrooms = facilityAvailableMore.data;
    } else {
        return facilityAvailableMore;
    }
    ...
    // Floor On
    let multiFloorOn = component.multiFloorOn(data.floorOn);
...
// Exporting Modules
module.exports = { readyToMoveSpace };
