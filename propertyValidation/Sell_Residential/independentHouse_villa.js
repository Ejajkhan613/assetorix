...
function independentHouse_villa(data) {
    ...
    // Amenities ( Optional )
    if (data.amenities) {
        const list = ["Maintenance Staff", "Rain Water Harvesting", "Water Storage", "Security / Fire Alarm", "Visitor Parking", "Park", "Intercom Facility", "Feng Shui / Vaastu Compliant", "Private Garden / Terrace", "Lift"];
    ...
    if (data.propertyFeatures) {
        const list = ["High Ceiling Height", "False Ceiling Lighting", "Piped-gas", "Internet / wi-fi connectivity", "Centrally Air Conditioned", "Water Purifier", "Recently Renovated", "Natural Light", "Airy Roooms", "Spacious Interiors"];
    ...
    if (data.society_buildingFeatures) {
        const list = ["Fitness Centre / GYM", "Swimming Pool", "Club house / Community Center", "Security Personnel"];
